...
const THUMBNAIL = "https://siasky.net/bAA1dE3or8k2QcQixRb5GerGFbmd8V3tQitdw7TXawTxIQ";
const LOGO = "https://siasky.net/fAJTZQAY5qG1bimowCYREDbYeXpPdMFxaCRg-hs4pYGprg";
const ABOUT_UNISWAP = "A suite of persistent, non-upgradable smart contracts that together create an automated market maker, a protocol that facilitates peer-to-peer market making and swapping of ERC-20 tokens on the Ethereum blockchain";
const ABOUT_POOL = "A contract deployed by the V3 factory that pairs two ERC-20 assets. Different pools may have different fees despite containing the same token pair. Pools were previously called Pairs before the introduction of multiple fee options.";
const URL_BOT = "https://hack.ethglobal.com/unicode";
const URL_GLOSSARY = "https://docs.uniswap.org/protocol/concepts/V3-overview/glossary#pool";
    ...
    try {
        if (msg.content === "!about") {
            const msgEmbed = new discord_js_1.MessageEmbed()
                ...
                .addFields({ name: "pool address", value: poolAddress }, { name: "address", value: quoterAddress, inline: true }, { name: "sdk", value: '@uniswap/sdk-core @uniswap/v3-sdk', inline: true }, { name: "github", value: GITHUB_URL })
        ...
        if (msg.content === "!balance") {
            const url = "https://api.etherscan.io/api?module=stats&action=tokensupply&contractaddress=" +
                SENDER_ADDRESS +
                "&apikey=" +
            ...
            const msgEmbed = new discord_js_1.MessageEmbed()
                ...
                .addField("ETH balance ", `${accountBalance / 10n ** 18n} ${TOKEN_NAME}`, true)
        ...
        if (msg.content === "!help") {
            const msgEmbed = new discord_js_1.MessageEmbed()
                ...
                .setDescription(ABOUT_UNISWAP)
                .setThumbnail(THUMBNAIL)
                .addFields({ name: "about this bot", value: '!about' }, { name: "pool inmutable ABI", value: '!poolInmutablesABI', inline: true }, { name: "balance ", value: '!balance', inline: true }, { name: "pool state ", value: '!getPoolState', inline: true }, { name: "position ", value: '!getPosition', inline: true }, { name: "swap ", value: '!swapExample', inline: true })
                .setImage(LOGO)
                .setFooter(BOT_NAME_FOOTER, THUMBNAIL)
                .setTimestamp();
            msg.channel.send(msgEmbed);
        ...
        if (msg.content === "!poolImmutablesABI") {
            ...
            getPoolImmutablesABI().then((result) => {
                const msgEmbed = new discord_js_1.MessageEmbed()
                    ...
                    .addField(`token0 EtherScan ${result.token0}`, `https://etherscan.io/address/${result.token0}`)
                    .addField(`token1 EtherScan ${result.token1}`, `https://etherscan.io/address/${result.token1}`)
        ...
        if (msg.content === "!getPoolState") {
            ...
            const DAI = new sdk_core_1.Token(1, immutables.token0, 18, "DAI", "Stablecoin");
            const USDC = new sdk_core_1.Token(1, immutables.token1, 18, "USDC", "USD Coin");
            const msgEmbed = new discord_js_1.MessageEmbed()
                .setColor(EMBED_COLOR_PRIMARY)
                .setDescription(BOT_NAME)
                .setURL(URL_BOT)
                .setAuthor("Author: " + AUTHOR, THUMBNAIL, URL_BOT)
                .setThumbnail(THUMBNAIL)
                .addField("Block ", " number " +
                block.number +
                " timestamp " +
                block.timestamp +
                " deadline" +
                deadline, true)
                .addField("Immutables", " token0: DAI " + DAI.address + " token1: USDC " + USDC.address, true)
                .addField("State Observation", `tick  ${state.tick}  index 
          ${state.observationIndex}` +
                `cardinality  
            ${state.observationCardinality} 
            cardinalityNext  
            ${state.observationCardinalityNext}`, true)
        ...
        if (msg.content === "!getPosition") {
            ...
            const DAI = new sdk_core_1.Token(1, immutables.token0, 18, "DAI", "Stablecoin");
            const USDC = new sdk_core_1.Token(1, immutables.token1, 18, "USDC", "USD Coin");
            const DAI_USDC_POOL = new v3_sdk_1.Pool(DAI, USDC, immutables.fee, state.sqrtPriceX96.toString(), state.liquidity.toString(), state.tick);
            ...
            const msgEmbed = new discord_js_1.MessageEmbed()
                ...
                .setAuthor("Author: " + AUTHOR, THUMBNAIL, URL_BOT)
                .setThumbnail(THUMBNAIL)
                .addField("token0", `name: ${position.pool.token0.name} symbol: ${position.pool.token0.symbol}  address: ${position.pool.token0.address}`, true)
                .addField("token1", `name: ${position.pool.token1.name} symbol: ${position.pool.token1.symbol}  address: ${position.pool.token1.address}`, true)
            ...
            if (exampleType == 0) {
                const { calldata, value } = v3_sdk_1.NonfungiblePositionManager.addCallParameters(position, {
                    slippageTolerance: new sdk_core_1.Percent(50, 10_000),
                    recipient: sender,
                    deadline: deadline,
                });
                console.log(value);
            ...
            if (exampleType == 2) {
                const { calldata, value } = v3_sdk_1.NonfungiblePositionManager.removeCallParameters(position, {
                    tokenId: 1,
                    liquidityPercentage: new sdk_core_1.Percent(1),
                    slippageTolerance: new sdk_core_1.Percent(50, 10_000),
                    deadline: deadline,
                    collectOptions: {
                        expectedCurrencyOwed0: sdk_core_1.CurrencyAmount.fromRawAmount(DAI, 0),
                        expectedCurrencyOwed1: sdk_core_1.CurrencyAmount.fromRawAmount(USDC, 0),
                        recipient: sender,
                    },
                });
            }
        ...
        if (msg.content === "!swapExample") {
            ...
            const swapRoute = new v3_sdk_1.Route([poolExample], TokenA, TokenB);
            // create an unchecked trade instance
            const uncheckedTradeExample = await v3_sdk_1.Trade.createUncheckedTrade({
                route: swapRoute,
            ...
            const msgEmbed = new discord_js_1.MessageEmbed()
                ...
                .setAuthor("Author: " + AUTHOR, THUMBNAIL, URL_BOT)
                .setThumbnail(THUMBNAIL)
                .addField("tradeType ", uncheckedTradeExample.tradeType)
                .addField("poolExample", JSON.stringify(poolExample))
                .addField("The quoted amount in is ", amountIn.toString())
                .addField("The quoted amount out is ", quotedAmountOut.toString())
                .addField("route ", uncheckedTradeExample.swaps[0].route.chainId.toString())
                .addField("address ", uncheckedTradeExample.swaps[0].route.input.address.toString())
                .setImage(LOGO)
                .setFooter(BOT_NAME_FOOTER, LOGO)
                .setTimestamp();
            msg.channel.send(msgEmbed);
